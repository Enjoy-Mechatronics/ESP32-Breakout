...
//IR sensor pins
const int rightIRpin = 18;
const int leftIRpin = 19;
...
void setup() {
  ...
  // Set up mDNS responder: "game.local" to access webpage
  if (!MDNS.begin("game")) {
...
void updateReadings() {
  ...
  char data[200];
  snprintf(data, 500, "{\"right\":%d, \"left\":%d}", digitalRead(rightIRpin), digitalRead(leftIRpin));
